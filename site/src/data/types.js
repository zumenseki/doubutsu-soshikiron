...
export const TYPES_SELF = {
  ...
  chimp: {
    ...
    weakness: '報酬で動かすほど「報酬が無いと動かない」に傾き（アンダーマイニング）、評価への不公平感もコストになりやすい。',
  ...
  bonobo: {
    ...
    weakness: '先回りで用意し続けると、相手が「自分でやってできた」機会を失い、優しさが自走を奪う側に回る。',
  ...
  hyena: {
    ...
    label: 'ムラありハイエナ型',
    healthy: false,
    finding: 'その時々で対応がブレやすい関わりに寄っている。',
    weakness: '対応がブレると相手は「何が正解か」を読めず、何度言っても定着しにくい。',
...
export const TYPES_FAMILY = {
  ookami: {
    ...
    weakness: '叱りで動かし続けると、回避や「どうせ無理」（学習性無力感）に傾きやすい。「止まった」は「育った」とは別。',
  ...
  ari: {
    ...
    weakness: '基準が「みんな」という外側にあるぶん、子ども自身の「やってみたらできた」という手応えが育ちにくい。',
  ...
  chimp: {
    ...
    weakness: 'ごほうびで釣るほど「ごほうびが無いとやらない」に傾き（アンダーマイニング）、一度あげた物を減らすと実際以上に強く嫌がる（損失回避）。',
  ...
  bonobo: {
    ...
    weakness: '先回りして代わりにやり続けると「自分でやってできた」経験が減り、優しさが「どうせ無理」（学習性無力感）を育てる側に回る。',
  ...
  hyena: {
    ...
    weakness: '同じ行動でも対応がバラつくと、子は「何が正解か」を読めず、言う回数を増やすほど混乱が増える。',
